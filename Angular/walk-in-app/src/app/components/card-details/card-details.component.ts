...
export class CardDetailsComponent implements OnInit {
  ...
  constructor(
    ...
    private spec: SpecJobPageService,
  ...
  ngOnInit(): void {
    ...
    this.spec.getJobCardData().subscribe((data) => {
      ...
      this.instructions = this.jobData.result1[this.id].generalInstruction
        .split('-')
        .slice(1);
      this.instructionsForExam = this.jobData.result1[this.id].instructions
        .split('-')
        .slice(1);
      this.MinSystemRequirement = this.jobData.result1[
        this.id
      ].systemRequirement
        .split('-')
        .slice(1);
      this.processInfo = this.jobData.result1[this.id].processInfo
        .split('.')
        .slice(1);
    ...
    await this.spec.getStatus(
      {
        ...
        "jobId": this.id+1
          ...
          if(data=="present")
          {
            ...
            this.router.navigate(['/hallTicket',this.id+1]);
          }
    });
